...
const CompletedTasksPage: React.FC = () => {
  ...
  if (loading) return <p className="text-center text-gray-500 mt-8">Loading completed tasks...</p>;
  if (error) return <p className="text-center text-red-500 mt-8">{error}</p>;

  return (
    <div className="mt-6">
      <h2 className="text-2xl font-semibold mb-4 text-gray-700">Completed Tasks</h2>
      <TaskList tasks={tasks} onTaskUpdated={fetchCompletedTasks} onTaskDeleted={fetchCompletedTasks} />
    </div>
  );
};
